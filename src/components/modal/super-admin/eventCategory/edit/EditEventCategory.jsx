...
import { reqtoSuperAdminEditEventCategory, reqtoSuperAdminGeteventCategory } from "../../../../../pages/redux-Toolkit/services/superadmin/SuperAdminServices";
...
const EditEventCategory = ({ show, handleClose, eventCategoryList, eventCategory }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        if (formData.eventCategoryImage) {
            payload.append("eventCategoryImage", formData.eventCategoryImage);
        }

        try {
            const res = await dispatch(
                reqtoSuperAdminEditEventCategory({ id, data: payload })
            ).unwrap();
    ...
    useEffect(() => {
        ...
        if (eventCategory) {
            setFormData(prev => ({
                ...
                eventCategoryImage: null,
                eventCategoryImagePreview: eventCategory?.image || null,
            }));
        }
    }, [eventCategory]);
    return (
        <>
            <Modal className="form" show={show} backdrop="static" centered>
                <div className="modal-header">
                    <h5 className="modal-title mb-4">Edit Event Category</h5>
