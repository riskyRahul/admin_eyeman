...
import { reqtoSuperAdminCountriesWiseCountry, reqtoSuperAdminEditPalce, reqtoSuperAdminGetPlace } from "../../../../../pages/redux-Toolkit/services/superadmin/SuperAdminServices";
...
const EditPlace = ({ show, handleClose, GetPlaceList, place }) => {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const res = await dispatch(
                reqtoSuperAdminEditPalce({ id, data: payload })
            ).unwrap();
    ...
    useEffect(() => {
        if (place) {
            setFormData(prev => ({
                ...
                name: place?.name || "",
                placeImage: null,
                placeImagePreview: place?.image || null,
            }));

            setContinentId(place?.continentId || "");
            setCountryId(place?.countryId || "");

            if (place?.continentId) {
                dispatch(reqtoSuperAdminCountriesWiseCountry(place.continentId))
                    .unwrap()
                    .then((res) => {
                        setCountryList(res.data);
                    })
        ...
        <>
            <Modal className="form" show={show} backdrop="static" centered>
                <div className="modal-header">
                    <h5 className="modal-title mb-4">Edit Place</h5>
