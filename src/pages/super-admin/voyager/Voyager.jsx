...
const Voyager = () => {
  ...
  const columns = [
    ...
    },
    {
      ...
      cell: (row) => {
        ...
        const deleteReason = row?.deleteReason || "";
        ...
        return (
          <span
            ...
            title={isDeleted && deleteReason ? ` ${deleteReason}` : ""}
            style={{
              cursor: isDeleted && deleteReason ? "pointer" : "default",
              color: isDeleted ? "#dc3545" : "#28a745",
              fontWeight: "600",
            }}
          >
  ...
  const filtered = (voyagerList || []).filter((i) => {
    const s =
      `${i.name} ${i.email} ${i.country} ${i.phone_number}`.toLowerCase();
